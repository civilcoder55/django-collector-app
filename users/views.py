        ...
        if form.is_valid():
            ...
            try:
                ...
                if device.verify_token(code):
                    # login the user if code is valid
                    login(request, user,
...
def profile(request):
    if request.method == 'POST':
        ...
        pic = forms.ProfileUpdateForm(
            request.POST, request.FILES, instance=request.user.profile_pic)
        if info.is_valid() and pic.is_valid():
            info.save()
            pic.save()
            messages.success(
                request, 'Your information was successfully updated!')
            return redirect('profile')
        error = list(info.errors.as_data()[
                     list(info.errors.as_data().keys())[0]][0])
        messages.error(request, error[0])
        return redirect('profile')
...
def password(request):
    if request.method == 'POST':
        ...
        error = list(password.errors.as_data()[
                     list(password.errors.as_data().keys())[0]][0])
        messages.error(request, error[0])
        return redirect('password')
