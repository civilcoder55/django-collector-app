...
class TwoFactorAuth(forms.Form):
    ...
    code = forms.CharField(
        widget=forms.NumberInput(
            attrs={'maxlength': "6", 'class': "form__input",
                   'placeholder': 'Authentication code', 'required': True,
                   'oninput':
                   "javascript: if (this.value.length > this.maxLength) \
...
class ActiveTwoFactorAuth(forms.Form):
    code = forms.CharField(
        widget=forms.NumberInput(
            attrs={'maxlength': "6", 'class': "form__input", 'required': True,
                   'oninput':
                   "javascript: if (this.value.length > this.maxLength)\
                   this.value = this.value.slice(0, this.maxLength);", }))
...
class ResetPass(SetPasswordForm):
    ...
    new_password2 = forms.CharField(
        widget=forms.PasswordInput(
            attrs={'class': "form__input", 'placeholder': "Confirm Password",
                   'required': True}))


class ProfileUpdateForm(forms.ModelForm):
    pic = forms.ImageField(widget=forms.FileInput(
        attrs={"class": 'form__gallery-upload',
